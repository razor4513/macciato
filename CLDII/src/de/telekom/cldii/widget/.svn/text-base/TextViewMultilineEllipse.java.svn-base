 ...
 * enough space for the input string. In expanded mode, the widget takes up as
 * much height as necessary for the input string. You can toggle the mode to
 * expand/collapse the widget in-place, via a click-handler for example.
 * 
 ...
 * /view/LabelView.html
 * 
 * @author Mark Wyszomierski and Jun Chen, jambit GmbH (Modifications)
 */
public class TextViewMultilineEllipse extends View {
    ...
    private float mAddExtraLineSpace;

    private int oldBreakWidth = -1;
    ...
    public TextViewMultilineEllipse(Context context, AttributeSet attrs) {
        ...
        s = a.getString(R.styleable.TextViewMultilineEllipse_android_text);
        if (s != null)
            setText(s);

        final float scale = getContext().getResources().getDisplayMetrics().density;
        setTextSize(a.getDimensionPixelSize(R.styleable.TextViewMultilineEllipse_android_textSize, (int) (14 * scale)));

        setTextColor(a.getInt(R.styleable.TextViewMultilineEllipse_android_textColor, 0xff000000));

        s = a.getString(R.styleable.TextViewMultilineEllipse_ellipsis);
        if (s != null)
            setEllipsis(s);

        s = a.getString(R.styleable.TextViewMultilineEllipse_colorEllpsizeMore);
        if (s != null)
            setEllipsisMore(s);

        setAdditionalLineSpace(a.getFloat(R.styleable.TextViewMultilineEllipse_additonalLineSpace, 0f) * scale);

        setMaxLines(a.getInt(R.styleable.TextViewMultilineEllipse_android_maxLines, 2));

        setColorEllpsizeMore(a.getInt(R.styleable.TextViewMultilineEllipse_colorEllpsizeMore, 0xff000000));

        setRightAlignEllipsizeMoreString(a.getBoolean(
                R.styleable.TextViewMultilineEllipse_rightAlignEllipsizeMoreString, false));

        a.recycle();
    }

    public void setAdditionalLineSpace(float spacing) {
        if (mAddExtraLineSpace != spacing) {
            mAddExtraLineSpace = spacing;
            requestLayout();
            invalidate();
        }
    }
    ...
    private int measureHeight(int measureSpec) {
        ...
        if (specMode == MeasureSpec.EXACTLY) {
            ...
            result = (int) (numLines * (-mAscent + mTextPaint.descent() + mAddExtraLineSpace)) + getPaddingTop()
                    + getPaddingBottom();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (pair[0] < pair[1] || mText.substring(pair[0], pair[1] + 1).length() > 0) {

                // Remove the space at the beginning of a line
                if (mText.charAt(pair[0]) == ' ') {
                    canvas.drawText(mText, pair[0] + 1, pair[1] + 1, x, y, mTextPaint);
                } else {
                    ...
                    if (breaker.getRequiredEllipsis()) {
                        ...
                        if (mDrawEllipsizeMoreString) {
                            ...
                            if (mRightAlignEllipsizeMoreString) {
                                ...
                                canvas.drawText(mStrEllipsisMore,
                ...
                y += (-mAscent + mTextPaint.descent() + mAddExtraLineSpace);
    ...
    private int breakWidth(int availableWidth) {
        ...
        oldBreakWidth = widthUsed + getPaddingLeft() + getPaddingRight();
    ...
    private static class LineBreaker {
         ...
         *            Current paint object with styles applied to it.
         */
        public int breakText(String input, String ellipsis, String ellipsisMore, int maxLines, int maxWidth,
                TextPaint tp) {
            mLines.clear();
            mRequiredEllipsis = false;
