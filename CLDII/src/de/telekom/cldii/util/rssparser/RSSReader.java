...
public class RSSReader implements java.io.Closeable {
  ...
  public RSSFeed load(String uri) throws RSSReaderException {
    ...
    try {
      ...
      if (elements != null && elements.length > 0 && elements[elements.length - 1].getParameterByName("charset") != null) {
          String encoding = elements[elements.length - 1].getParameterByName("charset").getValue();
          feed = parser.parse(feedStream, encoding);
      } else
